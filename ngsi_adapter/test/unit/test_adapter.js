...
suite('adapter', function () {
    ...
    test('response_includes_autogenerated_correlator', function () {
        var response = {
            writeHead: sinon.stub(),
            end: sinon.stub()
        };
        ...
        this.httpListener(this.request, response);
        assert(response.writeHead.calledOnce);
        assert.equal(response.writeHead.args[0][0], 200);
        assert(response.writeHead.args[0][1][common.correlatorHttpHeader]);
    });

    test('response_includes_same_request_correlator', function () {
        var response = {
            writeHead: sinon.stub(),
            end: sinon.stub()
        };
        var corr = 'correlator-1234';
        this.request.url = this.baseurl + '/' + this.resource + '?id=id&type=type';
        this.request.headers[common.correlatorHttpHeader.toLowerCase()] = corr;
        this.httpListener(this.request, response);
        assert(response.writeHead.calledOnce);
        assert.equal(response.writeHead.args[0][0], 200);
        assert.equal(response.writeHead.args[0][1][common.correlatorHttpHeader], corr);
    });
